    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      @forelse($units as $unit)
        <a href="{{ route('public.athletes.by-unit', $unit) }}"
          class="group block rounded-xl border border-slate-200 bg-white p-6 shadow-sm hover:border-blue-500 hover:ring-1 hover:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 transition-all">

          <div class="flex items-center justify-between">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white group-hover:text-blue-600">
              {{ $unit->name }}
            </h3>
            <span
